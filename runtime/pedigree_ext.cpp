...
void __cilkrts_extend_spawn(__cilkrts_worker *w, void **parent_extension,
    ...
    *child_extension = frame;
    ...
    //   leb8-single, leb8-anchor: continuation = right child, spawned child =
    //                left child; sync uses restore_idx stored on this frame
    //                when it was spawned
    ...
    // Spill before the copy, so the child and the continuation share any
    // chunk it makes (see os_label::prepare_spawn).
    parent_frame->label.prepare_spawn();
#endif
    frame->label = parent_frame->label;
    ...
    frame->tool_word = fresh_tool_word(frame->label);
    parent_frame->tool_word = fresh_tool_word(parent_frame->label);
...
#endif
    frame->tool_word = fresh_tool_word(frame->label); // the label changed
